...
spsim<-function(N=900, R=30, seed=123579, seed2=242398, tol=1e-10,
  ...
  #res<-foreach(i=1:R) %dopar% {  
    ...
    sar.res$b.direct[1,]<-sar.imp$direct_sum$statistics[,1]
    sar.res$sd.direct[1,]<-sar.imp$direct_sum$statistics[,2]
    
    sem.res$b.direct[1,]<-sem.mod$coefficients[-1]
    sem.res$sd.direct[1,]<-sem.mod$rest.se[-1]
    
    sac.res$b.direct[1,]<-sac.imp$direct_sum$statistics[,1]
    sac.res$sd.direct[1,]<-sac.imp$direct_sum$statistics[,2]
    
    sdm.res$b.direct[1,]<-sdm.imp$direct_sum$statistics[,1]
    sdm.res$sd.direct[1,]<-sdm.imp$direct_sum$statistics[,2]
    ...
    sar.res$b.indirect[1,]<-sar.imp$indirect_sum$statistics[,1]
    sar.res$sd.indirect[1,]<-sar.imp$indirect_sum$statistics[,2]
    
    sac.res$b.indirect[1,]<-sac.imp$indirect_sum$statistics[,1]
    sac.res$sd.indirect[1,]<-sac.imp$indirect_sum$statistics[,2]
    
    sdm.res$b.indirect[1,]<-sdm.imp$indirect_sum$statistics[,1]
    sdm.res$sd.indirect[1,]<-sdm.imp$indirect_sum$statistics[,2]
