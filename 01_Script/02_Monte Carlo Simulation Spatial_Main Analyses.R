...
comb_mat11<-expand.grid(l)

colnames(comb_mat11)<-c("rho", "delta", "lambda", "gamma", "theta")
comb_mat11$delta<-as.character(comb_mat11$delta)
comb_mat11$theta<-as.character(comb_mat11$theta)

save(comb_mat11, file="Combinations11.RData")

load("Combinations11.RData")

### To avoid duplicated simulations
load("Combinations4.RData") 
# load("Combinations3.RData") 
# load("Combinations4.RData") 
# load("Combinations5.RData") 
# load("Combinations6.RData") 
# load("Combinations7.RData") 
# load("Combinations8.RData") 
# load("Combinations9.RData") 
# load("Combinations10.RData") 
# 
# comb_mat3_10<-rbind(comb_mat3, comb_mat4, comb_mat5, comb_mat6, comb_mat7, 
#                     comb_mat8, comb_mat9, comb_mat10)
# 
# comb_mat3_10$sim<-rep(c(3:10), each=32)
# comb_mat3_10$n<-rep(c(1:32), times=8)
...
for(i in k){
  ...
  sim11<-NA # avoid using the last sim (in case of error)
  
  ### Search for duplicates
  
  # dupl<-apply(comb_mat3_10[,1:5], 1, FUN= function(x) all(comb_mat11[i,]==x))
  # if(any(dupl)==T){
    # j<-which(dupl==T)
    # dupli<-comb_mat3_10[j[1],6:7]
    # 
    # load(paste0("Simulation", dupli[1], "_", dupli[2], ".RData", sep=""))
    # tmp<-get(paste0("sim", dupli[1],"_", dupli[2], sep=""))
    # assign(paste0("sim11_", i, sep=""), tmp)
    
    
  if(all(comb_mat11[i,]==comb_mat4[i,])){
    ...
    sim11<-spsim(N=900, R=1000, seed=1573262181, tol=1e-15,
                 ay=par$rho, ax1=deltai[1], ax2=deltai[2], au=par$lambda,
                 bx1=beta[1], bwx1=thetai[1], bx2=beta[2], bwx2=thetai[2],
                 bu=1, bwu=0, sdu=1,
                 omv1=par$gamma, womv1=0)
    
    assign(paste0("sim11_", i, sep=""), sim11)
  }
  
  
  save(list=c(paste0("sim11_", i, sep="")), file=paste0("Simulation11_", i, ".RData", sep=""))
...
for(i in k){
  load(paste("Simulation11_", i, ".RData", sep=""))
}
...
for(i in k){
  ...
  tmp.sim<-get(paste("sim11_", i, sep=""))
  
  p<-plot.sim(tmp.sim, xlim=c(-2, 3))
  
  name = paste(od, "sim11_", i, ".jpeg", sep="")
  jpeg(file = name, bg = "transparent", width = 2720, height = 1820, quality=100, family="CM Roman", res=300)
  par(mar=c(0,0,0,0))
  par(mfrow=c(1,1),oma=c(0,0,0,0))
...
res11_rmse_dir<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_rmse_dir)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_rmse_dir<-cbind(res11_rmse_dir,tmp)

for(i in 1:nrow(res11_rmse_dir)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_d[-seq(1, nrow(s$mat_d), by=3),]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_rmse_dir[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_rmse_dir[i, 6:ncol(res11_rmse_dir)]<-t(m[,2])
}
...
res11_rmse_dir1<-res11_rmse_dir[res11_rmse_dir$gamma==0,]
#res11_rmse_dir2<-res11_rmse_dir[res11_rmse_dir$gamma!=0,]

res11_rmse_dir1<-res11_rmse_dir1[,-which(names(res11_rmse_dir1)=="gamma")]
#res11_rmse_dir2<-res11_rmse_dir2[,-which(names(res11_rmse_dir2)=="gamma")]


res11_rmse_dir1[,1:4]<-apply(res11_rmse_dir1[,1:4], 2, FUN=function(x) as.character(x))
#res11_rmse_dir2[,1:4]<-apply(res11_rmse_dir2[,1:4], 2, FUN=function(x) as.character(x))
...
res11_rmse_ind<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_rmse_ind)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_rmse_ind<-cbind(res11_rmse_ind,tmp)

for(i in 1:nrow(res11_rmse_ind)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_sp[-seq(1, nrow(s$mat_d), by=3),]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_rmse_ind[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_rmse_ind[i, 6:ncol(res11_rmse_ind)]<-t(m[,2])
}


res11_rmse_ind<-res11_rmse_ind[,-which(names(res11_rmse_ind) %in% c("OLS_x1", "OLS_x2", "SEM_x1", "SEM_x2"))]

### Separate into omv and non-omv

res11_rmse_ind1<-res11_rmse_ind[res11_rmse_ind$gamma==0,]
#res11_rmse_ind2<-res11_rmse_ind[res11_rmse_ind$gamma!=0,]

res11_rmse_ind1<-res11_rmse_ind1[,-which(names(res11_rmse_ind1)=="gamma")]
#res11_rmse_ind2<-res11_rmse_ind2[,-which(names(res11_rmse_ind2)=="gamma")]


res11_rmse_ind1[,1:4]<-apply(res11_rmse_ind1[,1:4], 2, FUN=function(x) as.character(x))
#res11_rmse_ind2[,1:4]<-apply(res11_rmse_ind2[,1:4], 2, FUN=function(x) as.character(x))
...
res11_rmse_dir1<-res11_rmse_dir1[,-which(names(res11_rmse_dir1) %in% c("delta", "theta"))]
res11_rmse_ind1<-res11_rmse_ind1[,-which(names(res11_rmse_ind1) %in% c("delta", "theta"))]
...
header2<-paste(paste("$\\rho$", "$\\lambda$", sep=" & "), paste(rep("\\tc{$\\bm x_1$}  & \\tc{$\\bm x_2$}", 5), collapse=" & "), sep=" & " )

matout(res11_rmse_dir1, file=paste(od, "res11_rmse_dir1.tex", sep=""), digits=4, stars=T,
       caption="RMSE of direct impacts for different strengths of autocorrelation. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(0, 0)^\\intercal$, ${\\bm\\delta}=(0, 0)^\\intercal$, ${\\bm\\theta}=(0.1, 0.8)^\\intercal$.", 
       label="table:res11_dir",
       code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
       header1=header1, header2=header2,
       note="\\ensuremath{^{*}} Lowest RMSE for ${\\bm x}_k$ within the parameter combination. Number of observations=900, repetitions=1000.")
...
# matout(res11_rmse_dir2, file=paste(od, "res11_rmse_dir2.tex", sep=""), digits=4, stars=T,
#        caption="RMSE of direct impacts with omv. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(-0.3, 0)^\\intercal$.", 
#        label="table:res11",
#        code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
#        header1=header1, header2=header2,
...
matout(res11_rmse_ind1, file=paste(od, "res11_rmse_ind1.tex", sep=""), digits=4, stars=T,
       caption="RMSE of indirect impacts for different strengths of autocorrelation. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(0, 0)^\\intercal$, ${\\bm\\delta}=(0, 0)^\\intercal$, ${\\bm\\theta}=(0.1, 0.8)^\\intercal$.", 
       label="table:res11_ind",
       code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
       header1=header1, header2=header2,
       note="\\ensuremath{^{*}} Lowest RMSE for ${\\bm x}_k$ within the parameter combination. Number of observations=900, repetitions=1000.")
...
# matout(res11_rmse_ind2, file=paste(od, "res11_rmse_ind2.tex", sep=""), digits=4, stars=T,
#        caption="RMSE of indirect impacts with omv. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(-0.3, 0)^\\intercal$.", 
#        label="table:res11",
#        code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
#        header1=header1, header2=header2,
...
res11_bias_dir<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_bias_dir)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_bias_dir<-cbind(res11_bias_dir,tmp)

for(i in 1:nrow(res11_bias_dir)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_d[-seq(1, nrow(s$mat_d), by=3),]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_bias_dir[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_bias_dir[i, 6:ncol(res11_bias_dir)]<-t(m[,1])
}
...
res11_bias_dir1<-res11_bias_dir[res11_bias_dir$gamma==0,]
#res11_bias_dir2<-res11_bias_dir[res11_bias_dir$gamma!=0,]

res11_bias_dir1<-res11_bias_dir1[,-which(names(res11_bias_dir1)=="gamma")]
#res11_bias_dir2<-res11_bias_dir2[,-which(names(res11_bias_dir2)=="gamma")]


res11_bias_dir1[,1:4]<-apply(res11_bias_dir1[,1:4], 2, FUN=function(x) as.character(x))
#res11_bias_dir2[,1:4]<-apply(res11_bias_dir2[,1:4], 2, FUN=function(x) as.character(x))
...
res11_bias_ind<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_bias_ind)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_bias_ind<-cbind(res11_bias_ind,tmp)

for(i in 1:nrow(res11_bias_ind)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_sp[-seq(1, nrow(s$mat_d), by=3),]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_bias_ind[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_bias_ind[i, 6:ncol(res11_bias_ind)]<-t(m[,1])
}


res11_bias_ind<-res11_bias_ind[,-which(names(res11_bias_ind) %in% c("OLS_x1", "OLS_x2", "SEM_x1", "SEM_x2"))]

### Separate into omv and non-omv

res11_bias_ind1<-res11_bias_ind[res11_bias_ind$gamma==0,]
#res11_bias_ind2<-res11_bias_ind[res11_bias_ind$gamma!=0,]

res11_bias_ind1<-res11_bias_ind1[,-which(names(res11_bias_ind1)=="gamma")]
#res11_bias_ind2<-res11_bias_ind2[,-which(names(res11_bias_ind2)=="gamma")]


res11_bias_ind1[,1:4]<-apply(res11_bias_ind1[,1:4], 2, FUN=function(x) as.character(x))
#res11_bias_ind2[,1:4]<-apply(res11_bias_ind2[,1:4], 2, FUN=function(x) as.character(x))
...
### Remove constant cols
res11_bias_dir1<-res11_bias_dir1[,-which(names(res11_bias_dir1) %in% c("delta", "theta"))]
res11_bias_ind1<-res11_bias_ind1[,-which(names(res11_bias_ind1) %in% c("delta", "theta"))]
...
matout(res11_bias_dir1, file=paste(od, "res11_bias_dir1.tex", sep=""), digits=4, stars=T,
       caption="Bias of direct impacts for different strengths of autocorrelation. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(0, 0)^\\intercal$, ${\\bm\\delta}=(0, 0)^\\intercal$, ${\\bm\\theta}=(0.1, 0.8)^\\intercal$.", 
       label="table:res1",
       code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
       header1=header1, header2=header2,
       note="\\ensuremath{^{*}} Lowest bias for ${\\bm x}_k$ within the parameter combination. Number of observations=900, repetitions=1000.")
...
# matout(res11_bias_dir2, file=paste(od, "res11_bias_dir2.tex", sep=""), digits=4, stars=T,
#        caption="Bias of direct impacts with omv. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(-0.3, 0)^\\intercal$.", 
#        label="table:res11",
#        code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
#        header1=header1, header2=header2,
...
matout(res11_bias_ind1, file=paste(od, "res11_bias_ind1.tex", sep=""), digits=4, stars=T,
       caption="Bias of indirect impacts for different strengths of autocorrelation. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(0, 0)^\\intercal$, ${\\bm\\delta}=(0, 0)^\\intercal$, ${\\bm\\theta}=(0.1, 0.8)^\\intercal$.", 
       label="table:res11",
       code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
       header1=header1, header2=header2,
       note="\\ensuremath{^{*}} Lowest bias for ${\\bm x}_k$ within the parameter combination. Number of observations=900, repetitions=1000.")
...
# matout(res11_bias_ind2, file=paste(od, "res11_bias_ind2.tex", sep=""), digits=4, stars=T,
#        caption="Bias of indirect impacts with omv. ${\\bm \\beta}=(0.2, 0.5)^\\intercal$, ${\\bm \\gamma}=(-0.3, 0)^\\intercal$.", 
#        label="table:res11",
#        code_before=c(paste("\\begin{tabular}{llll", paste(rep("D{.}{.}{1.5}", 14), collapse=" "), "}", collapse=" ")),
#        header1=header1, header2=header2,
...
res11_sd_dir<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_sd_dir)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_sd_dir<-cbind(res11_sd_dir,tmp)

for(i in 1:nrow(res11_sd_dir)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_sd[-seq(1, nrow(s$mat_d), by=3),1]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_sd_dir[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_sd_dir[i, 6:ncol(res11_sd_dir)]<-t(m)
}
...
res11_sd_dir1<-res11_sd_dir[res11_sd_dir$gamma==0,]
#res11_sd_dir2<-res11_sd_dir[res11_sd_dir$gamma>0,]

res11_sd_dir1<-res11_sd_dir1[,-which(names(res11_sd_dir1) %in% c("gamma", "delta","theta"))]
#res11_sd_dir2<-res11_sd_dir2[,-which(names(res11_sd_dir2)=="gamma")]


res11_sd_dir1[,1:4]<-apply(res11_sd_dir1[,1:4], 2, FUN=function(x) as.character(x))
#res11_sd_dir2[,1:4]<-apply(res11_sd_dir2[,1:4], 2, FUN=function(x) as.character(x))
...
res11_sd_ind<-comb_mat11

tmp<-data.frame(matrix(NA, ncol=14, nrow=nrow(res11_sd_ind)))
names(tmp)<-c("OLS_x1", "OLS_x2", "SLX_x1", "SLX_x2", "SAR_x1", "SAR_x2", "SEM_x1", "SEM_x2", "SAC_x1",
              "SAC_x2", "SDM_x1", "SDM_x2", "SDEM_x1", "SDEM_x2")

res11_sd_ind<-cbind(res11_sd_ind,tmp)

for(i in 1:nrow(res11_sd_ind)){
  tmp.sim<-get(paste("sim11_", i, sep=""))
  par<-tmp.sim$parameters
  s<-summary.sim(tmp.sim)
  m<-s$mat_sd[-seq(1, nrow(s$mat_sd), by=3),2]
  ...
  # Test for correct row
  par2<-as.character(c(par[1],paste(par[2],par[3], sep=", "), par[4], par[11], paste(par[6],par[8], sep=", ")))
  if(!identical(as.character(res11_sd_ind[i, 1:5]), par2)){
    cat("\n", "Parameters not identical at ", i, "\n")
    break
  }
  
  res11_sd_ind[i, 6:ncol(res11_sd_ind)]<-t(m)
}


res11_sd_ind<-res11_sd_ind[,-which(names(res11_sd_ind) %in% c("OLS_x1", "OLS_x2", "SEM_x1", "SEM_x2"))]

### Separate into omv and non-omv

res11_sd_ind1<-res11_sd_ind[res11_sd_ind$gamma==0,]
#res11_sd_ind2<-res11_sd_ind[res11_sd_ind$gamma>0,]

res11_sd_ind1<-res11_sd_ind1[,-which(names(res11_sd_ind1) %in% c("gamma", "delta","theta"))]
#res11_sd_ind2<-res11_sd_ind2[,-which(names(res11_sd_ind2)=="gamma")]


res11_sd_ind1[,1:4]<-apply(res11_sd_ind1[,1:4], 2, FUN=function(x) as.character(x))
#res11_sd_ind2[,1:4]<-apply(res11_sd_ind2[,1:4], 2, FUN=function(x) as.character(x))
...
tmp<-res11_bias_dir1[,grep("x1", colnames(res11_bias_dir1))]
tmp_sd<-res11_sd_dir1[,grep("x1", colnames(res11_sd_dir1))]
...
names<-apply(res11_bias_dir1[,1:2], 1, function(x) paste(x, collapse="    "))
names<-paste(paste(c(1:16), ")", sep=""), names, sep="    ")
mf_dir1$Model<-rep(names,7)
mf_dir1$modelName<-rep(c("OLS", "SLX", "SAR", "SEM", "SAC", "SDM", "SDEM"), each=16)
...
tmp<-res11_bias_dir1[,grep("x2", colnames(res11_bias_dir1))]
tmp_sd<-res11_sd_dir1[,grep("x2", colnames(res11_sd_dir1))]
...
tmp<-res11_bias_ind1[,grep("x1", colnames(res11_bias_ind1))]
tmp_sd<-res11_sd_ind1[,grep("x1", colnames(res11_sd_ind1))]
...
names<-apply(res11_bias_ind1[,1:2], 1, function(x) paste(x, collapse="    "))
names<-paste(paste(c(1:16), ")", sep=""), names, sep="    ")
mf_ind1$Model<-rep(names,7)
mf_ind1$modelName<-rep(c("OLS", "SLX", "SAR", "SEM", "SAC", "SDM", "SDEM"), each=16)
...
tmp<-res11_bias_ind1[,grep("x2", colnames(res11_bias_ind1))]
tmp_sd<-res11_sd_ind1[,grep("x2", colnames(res11_sd_ind1))]
...
mf_all$y_min<-NA
mf_all$y_min[which(mf_all$eff %in% c("Direct x1", "Direct x2"))]<--0.7
mf_all$y_min[which(mf_all$eff %in% c("Indirect x1", "Indirect x2"))]<--16

mf_all$y_max<-NA
mf_all$y_max[which(mf_all$eff %in% c("Direct x1", "Direct x2"))]<-0.35
mf_all$y_max[which(mf_all$eff %in% c("Indirect x1", "Indirect x2"))]<-16
...
zp_all <- zp_all + facet_grid(. ~ eff, scales="free_x")
zp_all <- zp_all + geom_hline(yintercept = 0, colour = "black", lty = 2)
zp_all <- zp_all + scale_shape_manual(values=c(15,16,17,18,3,4,8)) + scale_size_manual(values=0.8)
zp_all <- zp_all + geom_pointrange(aes(x = Model, y = Coefficient, ymin =lb,
                                       ymax = ub, 0.2),
...
zp_all <- zp_all + theme(legend.text = element_text(size = 20),
                         ...
                         axis.title.x = element_text(size=20),
                         axis.title.y = element_text(size=20),
                         plot.title = element_text(size=20, margin = margin(t = 10, b = -19 ), hjust=-0.06),
                         strip.background =element_blank(),
                         strip.text = element_text(size=20, colour="black"))
zp_all <- zp_all + ggtitle(bquote(rho ~~~~~~ lambda)) 
zp_all <- zp_all + guides(fill=guide_legend(ncol = 1), colour = guide_legend(override.aes = list(linetype=0), reverse=T),
                          shape = guide_legend(reverse = T))
print(zp_all)
...
cairo_pdf(file=paste(od, "sim11_combined_1.pdf", sep=""), width=17.54, height=12.40, 
          bg = "white", family="CM Roman")
par(mar=c(0,0,0,0))
par(mfrow=c(1,1),oma=c(0,0,0,0))
zp_all
...
######################################
#### Combined Plot Direct Effects ####
######################################

#### Combine plots ####

mf_dir<-mf_all[which(mf_all$eff %in% c("Direct x1", "Direct x2")),]
...
mf_dir$lb<-mf_dir$Coefficient - mf_dir$SE*interval2
mf_dir$ub<-mf_dir$Coefficient + mf_dir$SE*interval2

mf_dir$ub[which(mf_dir$ub>mf_dir$y_max)]<-mf_dir$y_max[which(mf_dir$ub>mf_dir$y_max)]
mf_dir$lb[which(mf_dir$lb<mf_dir$y_min)]<-mf_dir$y_min[which(mf_dir$lb<mf_dir$y_min)]



# Plot
zp_dir <- ggplot(mf_dir, aes(colour = modelName, shape = modelName))
zp_dir <- zp_dir + facet_grid(. ~ eff, scales="free_x")
zp_dir <- zp_dir + geom_hline(yintercept = 0, colour = "black", lty = 2)
zp_dir <- zp_dir + scale_shape_manual(values=c(15,16,17,18,3,4,8)) + scale_size_manual(values=0.8)
zp_dir <- zp_dir + geom_pointrange(aes(x = Model, y = Coefficient, ymin =lb,
                                       ymax = ub, 0.2),
                                   lwd = 0.8, position = position_dodge(width = 0.6),
                                   fill ="black")
zp_dir <- zp_dir + coord_flip() + theme_bw()
#zp_dir <- zp_dir + ylim(xlim[1], xlim[2])
zp_dir <- zp_dir + geom_blank(aes(y = y_min)) + geom_blank(aes(y = y_max))
zp_dir <- zp_dir + scale_y_continuous(expand = c(0,0) )
zp_dir <- zp_dir + theme(legend.title = element_blank())
zp_dir <- zp_dir + labs(y="Bias", x=element_blank()) 
zp_dir <- zp_dir + theme(legend.text = element_text(size = 20),
                         legend.position="bottom",
                         legend.key=element_blank(),
                         axis.text.x = element_text(size=16),
                         axis.text.y = element_text(size=16, colour="black"),
                         ...
                         strip.text = element_text(size=20, colour="black"),
                         panel.grid.major = element_line(size = (0.5)),panel.grid.minor = element_blank())
zp_dir <- zp_dir + ggtitle(bquote(rho ~~~~~~ lambda)) 
zp_dir <- zp_dir + guides(fill=guide_legend(ncol = 1), colour = guide_legend(override.aes = list(linetype=0), reverse=T),
                          shape = guide_legend(reverse = T))
print(zp_dir)
...
cairo_pdf(file=paste(od, "sim11_combined_1_dir.pdf", sep=""), height=12.40, width=17.54, 
          bg = "white", family="CM Roman")
par(mar=c(0,0,0,0))
par(mfrow=c(1,1),oma=c(0,0,0,0))
zp_dir
...
########################################
#### Combined Plot Indirect Effects ####
########################################

#### Combine plots ####

mf_ind<-mf_all[which(mf_all$eff %in% c("Indirect x1", "Indirect x2")),]
...
# Remove OLS and SEM
mf_ind<-mf_ind[-which(mf_ind$modelName %in% c("OLS", "SEM")),]

# Extract color palette

g <- ggplot_build(zp_dir)
colour<-unlist(unique(g$data[[2]]["colour"]))
...
mf_ind$lb<-mf_ind$Coefficient - mf_ind$SE*interval2
mf_ind$ub<-mf_ind$Coefficient + mf_ind$SE*interval2

mf_ind$ub[which(mf_ind$ub>mf_ind$y_max)]<-mf_ind$y_max[which(mf_ind$ub>mf_ind$y_max)]
mf_ind$lb[which(mf_ind$lb<mf_ind$y_min)]<-mf_ind$y_min[which(mf_ind$lb<mf_ind$y_min)]



# Plot
zp_ind <- ggplot(mf_ind, aes(colour = modelName, shape = modelName))
zp_ind <- zp_ind + facet_grid(. ~ eff, scales="free_x")
zp_ind <- zp_ind + geom_hline(yintercept = 0, colour = "black", lty = 2)
zp_ind <- zp_ind + scale_shape_manual(values=c(15,16,17,3,4)) + scale_size_manual(values=0.8)
zp_ind <- zp_ind + geom_pointrange(aes(x = Model, y = Coefficient, ymin =lb,
                                       ymax = ub, 0.2),
                                   lwd = 0.8, position = position_dodge(width = 0.6),
                                   fill ="black")
zp_ind <- zp_ind + coord_flip() + theme_bw()
zp_ind <- zp_ind + scale_colour_manual(values=unname(colour[rev(c(2,3,5,6,7))]))
#zp_ind <- zp_ind + ylim(xlim[1], xlim[2])
zp_ind <- zp_ind + geom_blank(aes(y = y_min)) + geom_blank(aes(y = y_max))
zp_ind <- zp_ind + scale_y_continuous(expand = c(0,0), breaks = c(-15, -10, -5, 0, 5, 10, 15))
zp_ind <- zp_ind + theme(legend.title = element_blank())
zp_ind <- zp_ind + labs(y="Bias", x=element_blank()) 
zp_ind <- zp_ind + theme(legend.text = element_text(size = 20),
                         legend.position="bottom",
                         legend.key=element_blank(),
                         axis.text.x = element_text(size=16),
                         axis.text.y = element_text(size=16, colour="black"),
                         ...
                         strip.text = element_text(size=20, colour="black"),
                         panel.grid.major = element_line(size = (0.5)),panel.grid.minor = element_blank())
zp_ind <- zp_ind + ggtitle(bquote(rho ~~~~~~ lambda)) 
zp_ind <- zp_ind + guides(fill=guide_legend(ncol = 1), colour = guide_legend(override.aes = list(linetype=0), reverse=T),
                          shape = guide_legend(reverse = T))
print(zp_ind)
...
cairo_pdf(file=paste(od, "sim11_combined_1_ind.pdf", sep=""), height=12.40, width=17.54, 
          bg = "white", family="CM Roman")
par(mar=c(0,0,0,0))
par(mfrow=c(1,1),oma=c(0,0,0,0))
zp_ind
