...
spsim<-function(N=900, R=30, seed=123579, seed2=242398, tol=1e-10,
  ...
  ### Random number of neighbours per obs
  
  c<-round(runif(N, min=min+1, max=max+1),0)
  c<-c[which(cumsum(c)<N)]
  if((max(cumsum(c))+2)<=N){
    c<-c(c, (N-max(cumsum(c))))
  }
  
  # set up list
  l <- vector("list", length = max(cumsum(c)))
  nids<-rep(c(1:length(c)), c)
  for(k in 1:N){
    j<-nids[k]
    nbs<-which(nids==j)
    nbs<-nbs[-which(nbs==k)]
  ...
  ### Create fake raster to use attributes
  df.sp<-raster(nrows=length(l), ncols=1)
  #df.sp <- extent(df.sp)
  ...
  attributes(l)<-attributes(df.nb)
  
  df.listw<-nb2listw(l, style="W")
  ...
  for(i in 1:R){
    ...
    sar.res$b.direct[i,]<-sar.imp$direct_sum$statistics[,1]
    sar.res$sd.direct[i,]<-sar.imp$direct_sum$statistics[,2]
    
    sem.res$b.direct[i,]<-sem.mod$coefficients[-1]
    sem.res$sd.direct[i,]<-sem.mod$rest.se[-1]
    
    sac.res$b.direct[i,]<-sac.imp$direct_sum$statistics[,1]
    sac.res$sd.direct[i,]<-sac.imp$direct_sum$statistics[,2]
    
    sdm.res$b.direct[i,]<-sdm.imp$direct_sum$statistics[,1]
    sdm.res$sd.direct[i,]<-sdm.imp$direct_sum$statistics[,2]
    ...
    sar.res$b.indirect[i,]<-sar.imp$indirect_sum$statistics[,1]
    sar.res$sd.indirect[i,]<-sar.imp$indirect_sum$statistics[,2]
    
    sac.res$b.indirect[i,]<-sac.imp$indirect_sum$statistics[,1]
    sac.res$sd.indirect[i,]<-sac.imp$indirect_sum$statistics[,2]
    
    sdm.res$b.indirect[i,]<-sdm.imp$indirect_sum$statistics[,1]
    sdm.res$sd.indirect[i,]<-sdm.imp$indirect_sum$statistics[,2]
